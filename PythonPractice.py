...
firstLastList = list(input("Give me a list to pull the first and last entries from: "))

def firstAndLast(list):

    return [list[0], list[-1]]

print(firstAndLast(firstLastList))

# Exercise 13

fibNum = int(input("Give me the length of a fibonacci sequence you'd like to see: "))
...
dupList = list(input("Give me a list and I'll remove the duplicates: "))

def dupListFunc(dupList):
    revisedList = []

    for item in dupList:
        if item not in revisedList:
            revisedList.append(item)
